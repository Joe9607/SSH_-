          ...
          <TD vAlign=top>
            ...
            background=qtimages/zjgdj_79.gif border=0>
              ...
              <TR>
               ...
               检查名称:<input name="jname"  type="text" value="${jname }"/> 
                ...
                <TD style="PADDING-TOP: 8px" vAlign=top height=185><TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
                    ...
                    <TR>
                      <TD style="PADDING-TOP: 8px" align=middle class=newsline><table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#FA9090" style="border-collapse:collapse">
                        ...
                        <c:forEach items="${record2list}" var="bean">
                        
                        <tr>
                         
                          <td width="10%">${bean.yuyue2.jiancha.jname }</td>
                          <td width="10%">${bean.yuyue2.jiancha.money }</td>
                          <td width="20%">${bean.yuyue2.riqi }--${bean.yuyue2.week }</td>
                          <td width="25%">${bean.ytime }</td>
                          <td width="15%">${bean.status }</td>
                         
                          <td width="10%" align="center">
                          
                          <c:if test="${riqi<bean.riqi}">
                           <c:if test="${bean.status=='预约成功'}">
                          <a href="indexmethod!record2delete?id=${bean.id }" onclick="{if(confirm('确认取消?')){return true;}return false;}">取消预定</a>
                          </c:if></c:if>
                          </td>
